...
export default function VisualizationArea({ algorithm, isPlaying, speed }) {
  ...
  return (
    <Card className="flex-1 p-6">
      ...
      <div className="flex h-[400px] items-end justify-center gap-1 rounded-lg bg-background/50 p-4">
        {array.map((value, idx) => (
          <div
            key={idx}
            ...
            style={{
              height: `${value}%`,
              backgroundColor: comparing.includes(idx) ? "hsl(var(--destructive))" : "hsl(var(--primary))",
            }}
          />
        ))}
      </div>
